<?php
/**
 * Agent CSV import / export tab markup.
 *
...
$export_fields   = function_exists('rch_agent_export_fields') ? rch_agent_export_fields() : [];
$export_defaults = function_exists('rch_agent_export_default_fields') ? rch_agent_export_default_fields() : [];
$has_export      = ! empty($export_fields);
if ($has_export) {
    wp_enqueue_script('jquery-ui-sortable');
}
?>
<div class="tab-content rch-agent-import rch-agent-tool">

    <?php if ($has_export) : ?>
    <div class="rch-agent-tool__switch" role="tablist" aria-label="<?php esc_attr_e('Import or export agents', 'rechat-plugin'); ?>">
        <button type="button" class="is-active" data-panel="import" role="tab" aria-selected="true"><?php esc_html_e('Import', 'rechat-plugin'); ?></button>
        <button type="button" data-panel="export" role="tab" aria-selected="false"><?php esc_html_e('Export', 'rechat-plugin'); ?></button>
    </div>
    <?php endif; ?>

    <!-- ══ IMPORT PANEL ══ -->
    <div class="rch-agent-tool__panel" data-panel="import">
        ...
        <details class="rch-agent-import__guide-details">
            <summary><?php esc_html_e('How to read the CSV', 'rechat-plugin'); ?></summary>
    ...
    <?php if ($has_export) : ?>
    <!-- ══ EXPORT PANEL ══ -->
    <div class="rch-agent-tool__panel" data-panel="export" hidden>
        <div class="rch-agent-import__hero">
            <div>
                <p class="rch-agent-import__eyebrow"><?php esc_html_e('Agents', 'rechat-plugin'); ?></p>
                <h2><?php esc_html_e('Export agents', 'rechat-plugin'); ?></h2>
                <p class="rch-agent-import__lead">
                    <?php esc_html_e('Tick the fields you want, then drag rows to set the CSV column order — the top row is the first column. Array fields (offices, regions, testimonials) export as JSON.', 'rechat-plugin'); ?>
                </p>
            </div>
        </div>
        ...
        <section class="rch-agent-export">
            <form method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>" id="rch-agent-export-form">
                ...
                <div class="rch-agent-export__toolbar">
                    <button type="button" class="button button-small rch-agent-export__select" data-check="1"><?php esc_html_e('Select all', 'rechat-plugin'); ?></button>
                    <button type="button" class="button button-small rch-agent-export__select" data-check="0"><?php esc_html_e('Select none', 'rechat-plugin'); ?></button>
                    <button type="button" class="button button-small rch-agent-export__reset"><?php esc_html_e('Reset order', 'rechat-plugin'); ?></button>
                    <span class="rch-agent-export__count" id="rch-export-count"></span>
                </div>

                <ul class="rch-agent-export__list" id="rch-agent-export-list">
                    <?php $i = 0; foreach ($export_fields as $key => $field) : ?>
                        <li class="rch-agent-export__row" data-order="<?php echo esc_attr((string) $i++); ?>">
                            <span class="rch-agent-export__handle dashicons dashicons-menu" aria-hidden="true"></span>
                            <label>
                                <input
                                    type="checkbox"
                                    name="fields[]"
                                    ...
                                    <?php checked(in_array($key, $export_defaults, true)); ?>
                                />
                                <span><?php echo esc_html($field['label']); ?> <code><?php echo esc_html($key); ?></code></span>
                            </label>
                        </li>
                    <?php endforeach; ?>
                </ul>

                <div class="rch-agent-export__preview">
                    <strong><?php esc_html_e('CSV columns (in order):', 'rechat-plugin'); ?></strong>
                    <span class="rch-agent-export__preview-cols" id="rch-export-preview"></span>
                </div>

                <button type="submit" class="button button-primary button-hero">
                    <?php esc_html_e('Export CSV', 'rechat-plugin'); ?>
                </button>
            </form>
        </section>
    </div><!-- /export panel -->
    ...
    jQuery(function ($) {
        // Panel switcher.
        $('.rch-agent-tool__switch button').on('click', function () {
            var panel = $(this).data('panel');
            $('.rch-agent-tool__switch button').removeClass('is-active').attr('aria-selected', 'false');
            $(this).addClass('is-active').attr('aria-selected', 'true');
            $('.rch-agent-tool__panel').attr('hidden', true);
            $('.rch-agent-tool__panel[data-panel="' + panel + '"]').removeAttr('hidden');
        });

        var $list = $('#rch-agent-export-list');
        ...
        function updatePreview() {
            ...
            $('#rch-export-preview').text(cols.length ? cols.join('  ,  ') : '<?php echo esc_js(__('None selected — defaults will be used.', 'rechat-plugin')); ?>');
            $('#rch-export-count').text(cols.length + ' / ' + total + ' <?php echo esc_js(__('selected', 'rechat-plugin')); ?>');
        }

        $('.rch-agent-export__select').on('click', function () {
            $list.find('input[type="checkbox"]').prop('checked', $(this).data('check') == 1);
            updatePreview();
        });

        $('.rch-agent-export__reset').on('click', function () {
            var rows = $list.children('.rch-agent-export__row').get();
            rows.sort(function (a, b) {
                return parseInt(a.getAttribute('data-order'), 10) - parseInt(b.getAttribute('data-order'), 10);
            });
            $.each(rows, function (_, row) { $list.append(row); });
            updatePreview();
        });

        $list.on('change', 'input[type="checkbox"]', updatePreview);

        if ($.fn.sortable) {
            $list.sortable({
                items: '.rch-agent-export__row',
                handle: '.rch-agent-export__handle',
                placeholder: 'rch-agent-export__placeholder',
                helper: 'clone',
