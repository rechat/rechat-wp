<?php
/**
 * Office sub-site deploy wizard (Rechat → tab).
 *
 * Mirrors the agent wizard view (agent-site-deploy-wizard-tab.php). Root id + rch-wz-* markup are
 * kept identical so the shared CSS and the office JS (a config-swapped copy of the agent JS) work
 * unchanged. Differences: offices wording, office post list, no testimonials, default target scope
 * = office sub-sites.
 *
 * @package Rechat
 */
...
$bulk_agent_count = function_exists('rch_office_wizard_count_offices_with_subsites')
    ? rch_office_wizard_count_offices_with_subsites()
    : 0;
$manual_rows      = function_exists('rch_office_wizard_manual_field_defs')
    ? rch_office_wizard_manual_field_defs()
    : [];
$metabox_defs     = function_exists('rch_office_wizard_importable_field_defs_resolved')
    ? rch_office_wizard_importable_field_defs_resolved()
    : [];

$wz_resolved = function_exists('rch_multisite_resolve_theme_network_default_for_offices')
...
$wz_profile     = function_exists('rch_office_wizard_get_ui_profile')
    ? rch_office_wizard_get_ui_profile()
    : ['storage_primary' => '', 'storage_mirror' => null];
$wz_opt_primary = isset($wz_profile['storage_primary']) && is_string($wz_profile['storage_primary']) ? $wz_profile['storage_primary'] : '';
$wz_opt_mirror  = (isset($wz_profile['storage_mirror']) && is_string($wz_profile['storage_mirror'])) ? $wz_profile['storage_mirror'] : '';
...
<div class="tab-content rch-agent-wizard" id="rch-agent-site-wizard">
    <div class="rch-wz-shell">
        ...
        <header class="rch-wz-hero">
            ...
            <h2 class="rch-wz-hero__title"><?php esc_html_e('Office sub-site data wizard', 'rechat-plugin'); ?></h2>
            <p class="rch-wz-hero__lead">
                <?php
                printf(
                    /* translators: 1: theme name, 2: stylesheet directory slug */
                    esc_html__(
                        'Map main-site office data into %1$s theme options (%2$s), using the same keys as that theme’s Theme Settings screen. Nothing is saved on sub-sites until you deploy.',
                        'rechat-plugin'
                    ),
            ...
            <ol class="rch-wz-stepnav__list">
                <li class="rch-wz-stepnav__item">
                    <button type="button" class="rch-wz-stepnav__btn rch-wz-goto" data-step="1">
                        ...
                        <span class="rch-wz-stepnav__label"><?php esc_html_e('Scope & office', 'rechat-plugin'); ?></span>
                    </button>
                </li>
        ...
        <div class="rch-agent-wizard-panel rch-wz-card" data-step-panel="1">
            <div class="rch-wz-card__head">
                <h3 class="rch-wz-card__title"><?php esc_html_e('Scope & office', 'rechat-plugin'); ?></h3>
                <p class="rch-wz-card__subtitle"><?php esc_html_e('Choose one office or push the same configuration to every office sub-site.', 'rechat-plugin'); ?></p>
            </div>
            <div class="rch-wz-card__body">
                <fieldset class="rch-wz-fieldset rch-agent-wizard-scope">
                    ...
                    <label class="rch-wz-choice">
                        ...
                        <span class="rch-wz-choice__box">
                            <span class="rch-wz-choice__title"><?php esc_html_e('Single office', 'rechat-plugin'); ?></span>
                            <span class="rch-wz-choice__desc"><?php esc_html_e('Load one office for preview and deploy only to that office’s sub-site.', 'rechat-plugin'); ?></span>
                        </span>
                    </label>
                    <label class="rch-wz-choice">
                        <input type="radio" name="rch_wz_scope" id="rch-wz-scope-all" value="all" class="rch-wz-choice__input" />
                        <span class="rch-wz-choice__box">
                            <span class="rch-wz-choice__title"><?php esc_html_e('All office sub-sites', 'rechat-plugin'); ?></span>
                            <span class="rch-wz-choice__desc">
                                <?php
                                printf(
                                    /* translators: %d: number of offices with a sub-site */
                                    esc_html__('Run the same rules on every linked site (%d ready). Metabox values still come from each office.', 'rechat-plugin'),
                ...
                <div id="rch-wz-single-agent-wrap" class="rch-wz-stack">
                    <div class="rch-wz-field">
                        <label class="rch-wz-field__label" for="rch-wz-agent-select"><?php esc_html_e('Office', 'rechat-plugin'); ?></label>
                        <div class="rch-wz-field__row">
                            <select id="rch-wz-agent-select" class="rch-wz-select rch-wz-select--grow">
                                <option value=""><?php esc_html_e('Choose an office…', 'rechat-plugin'); ?></option>
                                <?php foreach ($agents as $aid) : ?>
                                    <option value="<?php echo esc_attr((string) $aid); ?>"><?php echo esc_html(get_the_title($aid)); ?></option>
                                <?php endforeach; ?>
                            </select>
                            <button type="button" class="button rch-wz-btn-secondary" id="rch-wz-load-agent"><?php esc_html_e('Load office', 'rechat-plugin'); ?></button>
        ...
        <div class="rch-agent-wizard-panel rch-wz-card" data-step-panel="2" hidden>
            <div class="rch-wz-card__head">
                ...
                <p class="rch-wz-card__subtitle">
                    <?php
                    esc_html_e(
                        'Fields are loaded automatically from the active office theme (themeoption.php keys and Theme Setting labels). When you add or remove options in the theme, reload this page to refresh the list. Pick a source: leave unchanged, set manually, or bind an office profile field. Nothing is written until you deploy.',
                        'rechat-plugin'
                    );
                    ?>
                </p>
                ...
                <?php if (empty($manual_rows)) : ?>
                    <div class="rch-wz-preview-empty" style="margin:16px;">
                        <p class="rch-wz-preview-empty__text">
                            <?php
                            esc_html_e(
                                'No theme options discovered for the selected default office theme. This usually means the theme does not store its settings in a WordPress option array, or it has never been saved yet. Add a rechat-agent-wizard.json manifest to the theme, or configure option names via rch_agent_wizard_storage_config / rch_agent_wizard_theme_storage_map.',
                                'rechat-plugin'
                            );
                            ?>
                        </p>
                                ...
                                <div class="rch-wz-theme-row__controls">
                                    <div class="rch-wz-field">
                                        ...
                                        <select class="rch-wz-select rch-wz-row-mode" data-theme-key="<?php echo esc_attr($mr['key']); ?>">
                                            ...
                                            <option value="meta"><?php esc_html_e('Office profile (metabox)', 'rechat-plugin'); ?></option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="rch-wz-row-manual-wrap rch-wz-stack" hidden>
                                        ...
                                        <?php if (in_array($mr['type'], ['text', 'textarea'], true)) : ?>
                                            <div class="rch-wz-tokens" aria-label="<?php esc_attr_e('Insert office placeholders', 'rechat-plugin'); ?>">
                                                <div class="rch-wz-tokens__label"><?php esc_html_e('Placeholders', 'rechat-plugin'); ?></div>
                                                <div class="rch-wz-tokens__list">
                                                    <button type="button" class="rch-wz-token" data-token="{$post_title}"><?php esc_html_e('Office name', 'rechat-plugin'); ?></button>
                                                    <?php foreach ($metabox_defs as $mk => $md) : ?>
            ...
            <div class="rch-wz-card__body">
                <fieldset class="rch-wz-fieldset rch-wz-bc-targets">
                    ...
                    <label class="rch-wz-choice rch-wz-choice--compact">
                        <input type="radio" name="rch_wz_bc_target" id="rch-wz-bc-target-agent" value="agent_only" class="rch-wz-choice__input" />
                        <span class="rch-wz-choice__box">
                            <span class="rch-wz-choice__title"><?php esc_html_e('Agent sub-sites only', 'rechat-plugin'); ?></span>
                        </span>
                    </label>
                    <label class="rch-wz-choice rch-wz-choice--compact">
                        <input type="radio" name="rch_wz_bc_target" id="rch-wz-bc-target-office" value="office_only" checked class="rch-wz-choice__input" />
                        <span class="rch-wz-choice__box">
        ...
        <div class="rch-agent-wizard-panel rch-wz-card" data-step-panel="<?php echo esc_attr($wz_broadcast_step ? '4' : '3'); ?>" hidden>
            ...
            <div class="rch-wz-card__body">
                <fieldset class="rch-wz-fieldset rch-wz-mw-targets">
                    <legend class="rch-wz-field__label"><?php esc_html_e('Target blogs', 'rechat-plugin'); ?></legend>
                    <label class="rch-wz-choice rch-wz-choice--compact">
                        <input type="radio" name="rch_wz_mw_target" id="rch-wz-mw-target-agent" value="agent_only" class="rch-wz-choice__input" />
                        <span class="rch-wz-choice__box">
                            <span class="rch-wz-choice__title"><?php esc_html_e('Agent sub-sites only', 'rechat-plugin'); ?></span>
                        </span>
                    </label>
                    <label class="rch-wz-choice rch-wz-choice--compact">
                        <input type="radio" name="rch_wz_mw_target" id="rch-wz-mw-target-office" value="office_only" checked class="rch-wz-choice__input" />
                        <span class="rch-wz-choice__box">
                            <span class="rch-wz-choice__title"><?php esc_html_e('Office sub-sites only', 'rechat-plugin'); ?></span>
                        </span>
                    </label>
